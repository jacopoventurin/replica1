...
for pos_fn, force_fn in zip(pos_fns, forces_fns):    
    ...
    for idx in tqdm(range(n_replicas)):
        np.save(f"{processed_pos}/temperature_{Temperatures[idx]:.0f}/coor_{i+delta}", pos[idx, :, protein_index, :]) 
        np.save(f"{processed_forces}/temperature_{Temperatures[idx]:.0f}/forces_{i+delta}", force[idx, :, protein_index, :]) 
