...
class ReplicaExchange:
    ...
    def _mix_states(self, mixing, random_number):
        ...
        print('check',self.energy_matrix_check[i, i], self.energy_matrix_check[j, j],self._compute_reduced_potential([sampler_state_i, thermo_state_i]),self._compute_reduced_potential([sampler_state_j, thermo_state_j]))
        # print(energy_ii,energy_jj,self._compute_reduced_potential([sampler_state_i, thermo_state_i]),self._compute_reduced_potential([sampler_state_j, thermo_state_j]))
        ...
        if random_number < np.exp(log_p_accept):
            ...
            print(thermo_state_i.temperature,thermo_state_j.temperature)

            self._thermodynamic_states[i], self._thermodynamic_states[j] = (
                self._thermodynamic_states[j],
                self._thermodynamic_states[i],
            )
            print(self._thermodynamic_states[i].temperature,self._thermodynamic_states[j].temperature)
        ...
        #         self.energy_matrix[[i, j]] = self.energy_matrix[[j, i]]
                self.energy_matrix_check[[i, j]] = self.energy_matrix_check[[j, i]]
            print('after swap',self.energy_matrix_check[i])
            self._compute_reduced_potential_matrix()
            print('after compute',self.energy_matrix_check[i])
    ...
    def _compute_reduced_potential_matrix(self):
        ...
        self.energy_matrix_check = np.zeros((self.n_replicas, self.n_replicas))
        inps = []
        inps_tmp = []
        for i_s, sampler_state in enumerate(self._replicas_sampler_states):
            for replica_j, thermo_state in enumerate(self._thermodynamic_states):
                ...
                i_t = self._temperature_list.index(thermo_state.temperature)
                inps_tmp.append((sampler_state, thermo_state, context, (i_s,i_t)))
                inps.append((sampler_state, thermo_state, context))
                # print((i_s,i_t),self._compute_reduced_potential(inps[-1]))

        outs = mpiplus.distribute(
            self._compute_reduced_potential, inps, send_results_to="all"
        )

        outs_tmp = mpiplus.distribute(
            self._compute_reduced_potential_tmp, inps_tmp, send_results_to="all"
        )
        ...
        for ii,out in enumerate(outs_tmp):
            energy = out[0]
            ind = out[1]
            i_s = ind[0]
            i_t = ind[1]
            self.energy_matrix_check[ind[0],ind[1]] = energy

            # print(i_s,i_t,energy,self.energy_matrix[i_s,i_t])
        # np.testing.assert_allclose(self.energy_matrix_check,self.energy_matrix)

    def _compute_reduced_potential_tmp(
        self, args
    ):  # ,sampler_state, thermo_state, context=None):
        comm = MPI.COMM_WORLD
        rank = comm.Get_rank()
